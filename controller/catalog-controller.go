...
func (ctrl *CatalogController) HealthCheck(c *gin.Context) {
...
func (ctrl *CatalogController) BatchGetMovies(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request body; 'ids' integer slice is required"})
		return
	}

	if len(req.IDs) > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Batch size cannot exceed 100 movie IDs"})
		return
	}

	movies, err := ctrl.CatalogService.GetMoviesByIDs(req.IDs)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch batch movies"})
		return
	}
